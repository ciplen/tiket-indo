 ...
 * @author amrullah
 */
public class RptUserList extends RptAbstractXlsx {

	private TableData<DtoUserList> data;
	private int startIndex;
	private int maxRecord;

	public RptUserList(TableData<DtoUserList> data, int startIndex, int maxRecord) {
		this.data = data;
	...
	protected List<ExcelHeaderColumn> getColumns() {
		...
		lst.add(new ExcelHeaderColumn("Username", 200));
		lst.add(new ExcelHeaderColumn("Email", 200));
		lst.add(new ExcelHeaderColumn("Hak Akses", 200));
		return lst;
	}
	...
	protected List<List<Object>> getData() {
		...
		for (int i = 0; i < data.getRows().size(); i++) {
			DtoUserList dto = data.getRows().get(i);
			List<Object> row = new ArrayList<>();
			row.add(startIndex + i + 1);
			row.add(dto.username);
			row.add(dto.email);
			row.add(dto.role);
			rows.add(row);
		}
